    ...
    def load_train_val_test_split(self):
        ...
        if self.client_id is None and len(self.args.dataset) == 1: # server
            ...
            for client_id in range(self.args.num_clients):
                ...
                glb_merged_edge_index = torch.load(glb_merged_edge_index_path, weights_only=False)
                merged_edge_label = torch.load(merged_edge_label_path, weights_only=False)
                merged_edge_train_mask = torch.load(merged_edge_train_path, weights_only=False)
                merged_edge_val_mask = torch.load(merged_edge_val_path, weights_only=False)
                merged_edge_test_mask = torch.load(merged_edge_test_path, weights_only=False)
            ...
            if osp.exists(forward_data_path) and osp.exists(merged_edge_index_path) and osp.exists(merged_edge_label_path) \
                ...
                forward_data = torch.load(forward_data_path, weights_only=False)
                merged_edge_index = torch.load(merged_edge_index_path, weights_only=False)
                merged_edge_label = torch.load(merged_edge_label_path, weights_only=False)
                merged_edge_train_mask = torch.load(merged_edge_train_path, weights_only=False)
                merged_edge_val_mask = torch.load(merged_edge_val_path, weights_only=False)
                merged_edge_test_mask = torch.load(merged_edge_test_path, weights_only=False)
